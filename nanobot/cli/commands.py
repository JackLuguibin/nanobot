...
console_app = typer.Typer(help="Web console")
...
def console_start(
    ...
    # Check if config exists when gateway is needed
    if with_gateway:
        from nanobot.config.loader import get_config_path, load_config, save_config
        from nanobot.config.schema import Config
        ...
        if not config_path.exists():
            console.print("[yellow]Config not found. Running onboard...[/yellow]")
            config = Config()
            save_config(config)
            console.print(f"[green]✓[/green] Created config at {config_path}")
            ...
            sync_workspace_templates(workspace)
            console.print("[green]✓[/green] Onboard complete!\n")

    console.print(f"{__logo__} Starting nanobot console on {host}:{port}...")

    # Check if frontend is built - console is in project root, not in nanobot package
    console_web = Path(__file__).parent.parent.parent / "console" / "web"
    ...
    if with_gateway:
        console.print(f"[green]✓[/green] Starting gateway on port {gateway_port}...")
        ...
        def run_gateway():
            ...
            try:
                ...
                console.print("[yellow]Starting in limited mode (no agent)[/yellow]")
                provider = None
    ...
    try:
        ...
        import uvicorn
        # console is in project root, not in nanobot package
        ...
        console.print("Install console dependencies: pip install nanobot-ai[console]")
        raise typer.Exit(1)


@console_app.command("dev")
def console_dev(
    port: int = typer.Option(18791, "--port", "-p", help="Console server port"),
):
    """Start the nanobot console in development mode."""
    import subprocess
    from pathlib import Path
    ...
    console.print(f"{__logo__} Starting console in development mode...")
...
def console_build():
    ...
    console.print(f"{__logo__} Building console frontend...")

    try:
        subprocess.run(["npm", "install"], cwd=console_web, check=True)
        subprocess.run(["npm", "run", "build"], cwd=console_web, check=True)
        console.print("[green]✓[/green] Console built successfully")
    except subprocess.CalledProcessError as e:
        console.print(f"[red]Build failed: {e}[/red]")
        raise typer.Exit(1)
...
def console_run(
    ...
    # Check if config exists, if not run onboard first
    from nanobot.config.loader import get_config_path, load_config, save_config
    from nanobot.config.schema import Config
    from nanobot.utils.helpers import get_workspace_path, sync_workspace_templates
    ...
    console.print(f"{__logo__} Starting nanobot full stack...")
    ...
    console.print(f"[green]✓[/green] Starting console API on port {console_port}...")
    ...
    console_thread = threading.Thread(target=run_console_server, daemon=True)
    ...
    if frontend:
        ...
        console.print(f"[green]✓[/green] Starting frontend dev server on port 3000...")
        ...
        console.print("\n" + "=" * 50)
        console.print(f"{__logo__} Nanobot is running!")
        console.print("=" * 50)
        console.print(f"  Gateway:  http://localhost:{gateway_port}")
        console.print(f"  Console API: http://localhost:{console_port}")
